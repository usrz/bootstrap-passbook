const BarcodeExport = (function () {

  /* ======================================================================= */
  /* Our "Barcode" object                                                    */
  /* ======================================================================= */

  const Barcode = function (type, code, parameters) {
    const _function = __formats[type];

    if (_function == null) {
      throw new Error(`Unsupported barcode type "${type}"`);
    }

    const _barcode = _function(code, parameters);
  ...
  Barcode.prototype.createCanvas = function (scaleX, scaleY) {
    const scale = normalizeScale(scaleX, scaleY);
    ...
    for (let r = 0, y = 0; r < this.rows; r++, y += scale.y) {
      for (let c = 0, x = 0; c < this.cols; c++, x += scale.x) {
        if (this.data[r][c]) {
  ...
  Barcode.prototype.createImageData = function (scaleX, scaleY) {
    ...
    const _cols = this.cols;
    return __create_image_data(_cols * scale.x, _rows * scale.y, (x, y) => {
      const c = Math.floor(x / scale.x);
      const r = Math.floor(y / scale.y);
  ...
  +(function () {
    ...
    libbcmath._bc_simp_mul = function (n1, n1len, n2, n2len, full_scale) {
      ...
      let n1ptr,
        n2ptr,
        pvptr; // char *n1ptr, *n2ptr, *pvptr;
      let n1end,
        n2end; // char *n1end, *n2end;        /* To the end of n1 and n2. */
      let indx,
        sum,
        prodlen; // int indx, sum, prodlen;
    ...
    libbcmath._bc_shift_addsub = function (accum, val, shift, sub) {
      let accp,
        valp; // signed char *accp, *valp;
      let count,
        carry; // int  count, carry;
    ...
    */
    ...
    const TEXT_SUBMODES = [[ // ALPHA --------------------------------------------------------------------------------
      0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4a, 0x4b, 0x4c, 0x4d, 0x4e, 0x4f,
      0x50, 0x51, 0x52, 0x53, 0x54, 0x55, 0x56, 0x57, 0x58, 0x59, 0x5a, 0x20, 0xFD, 0xFE, 0xFF],
    [ // LOWER --------------------------------------------------------------------------------
      0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6a, 0x6b, 0x6c, 0x6d, 0x6e, 0x6f,
      0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7a, 0x20, 0xFD, 0xFE, 0xFF],
    [ // MIXED --------------------------------------------------------------------------------
      0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x26, 0x0d, 0x09, 0x2c, 0x3a,
      0x23, 0x2d, 0x2e, 0x24, 0x2f, 0x2b, 0x25, 0x2a, 0x3d, 0x5e, 0xFB, 0x20, 0xFD, 0xFE, 0xFF],
    [ // PUNCTUATION --------------------------------------------------------------------------
      0x3b, 0x3c, 0x3e, 0x40, 0x5b, 0x5c, 0x5d, 0x5f, 0x60, 0x7e, 0x21, 0x0d, 0x09, 0x2c, 0x3a,
      0x0a, 0x2d, 0x2e, 0x24, 0x2f, 0x22, 0x7c, 0x2a, 0x28, 0x29, 0x3f, 0x7b, 0x7d, 0x27, 0xFF]];
     ...
     */
    const RSFACTORS = [[ // ECL 0 (2 factors) ------------------------------------------------------------------------------------
      0x01b, 0x395],                                                                                      //   2
    [ // ECL 1 (4 factors) ------------------------------------------------------------------------------------
     ...
     *================================================================================================================================================== */
    const _get_input_sequences = function (code) {
      const sequence_array = []; // array to be returned
      let numseq = [];
      ...
      for (let i = 0; i < numseq.length; i++) {
        const seq = numseq[i];
        const seqlen = seq[0].length;
        if (seq[1] > 0) {
          // extract text sequence before the number sequence
          const prevseq = code.substr(offset, (seq[1] - offset));
          let textseq = [];
          // get text sequences
          ...
          let txtoffset = 0;
          for (let j = 0; j < textseq.length; j++) {
            const txtseq = textseq[j];
            const txtseqlen = txtseq[0].length;
            if (txtseq[1] > 0) {
              // extract byte sequence before the text sequence
              const prevtxtseq = prevseq.substr(txtoffset, (txtseq[1] - txtoffset));
    ...
    const _get_compaction = function (mode, code, addmode) {
      addmode = addmode || true;
      const cw = []; // array of codewords to return
      switch (mode) {
        case 900: { // Text Compaction mode latch
          let submode = 0; // default Alpha sub-mode
          let txtarr = []; // array of characters and sub-mode switching characters
          var codelen = code.length;
          for (var i = 0; i < codelen; ++i) {
            const chval = _ord(code.charAt(i));
            var k;
    ...
    const _get_error_correction_level = function (ecl, numcw) {
      // get maximum correction level
      let maxecl = 8; // starting error level
      const maxerrsize = (928 - numcw); // available codewords for error
      while (maxecl > 0) {
        const errsize = (2 << ecl);
    ...
    const _intval = function (mixed_var, base) {
    ...
    const _binary17 = function (number) {
      const padded = `00000000000000000${new Number(number).toString(2)}`;
     ...
     */
    const pdf417_create = function (code, parameters) {
      let _ecl = -1;
      let _aspect_ratio = 2;
      if (parameters != null) {
      ...
      const sequence = _get_input_sequences(code);
      let codewords = []; // array of code-words
      for (let i = 0; i < sequence.length; i++) {
        const cw = _get_compaction(sequence[i][0], sequence[i][1], true);
      ...
      const ecl = _get_error_correction_level(_ecl, numcw);
      // number of codewords for error correction
      const errsize = (2 << ecl);
      // calculate number of columns (number of codewords per row) and rows
      const nce = (numcw + errsize + 1);
      let cols = Math.round((Math.sqrt(4761 + (68 * _aspect_ratio * nce)) - 69) / 34);
      // adjust cols
      if (cols < 1) {
        cols = 1;
      } else if (cols > 30) {
      ...
      const ecw = _get_error_correction_word(codewords, ecl);
      // add error correction codewords
      codewords = codewords.concat(ecw);
      // create our return structure
      const _barcode_array = {
        rows,
        cols: ((cols + 2) * 17) + 35,
        data: new Array(),
        parameters: {
      ...
      let cid = 0; // initial cluster
      // for each row
      for (let r = 0; r < rows; ++r) {
        // row start code
        let row = START_PATTERN;
        switch (cid) {
          case 0: {
    ...
    /*= =================================================================================================================================================*
     | QRCODE CREATION FUNCTION                                                                                                                         |
     *================================================================================================================================================== */
    const qrcode_create = (function () {

      /* Imported (reduced) pretty much statically from QR lib,
       * See at the bottom for our-style function */
      ...
      /*= ===============================================================================================================================================*
       | Actually create a new QRCode and return our consistent structure                                                                               |
       | "typeNumber" in the lib's terminology is what Wikipedia calls "version"... we follow that                                                      |
       *================================================================================================================================================ */
      return function (code, parameters) {
        let version = 4;
        let errorCorrectLevelNum = 1;
        let errorCorrectLevel = 'M';

        if (parameters != null) {
          version = Number(parameters.version) || 6;
          if (version < 1) version = 1;
          if (version > 10) version = 10;

          errorCorrectLevelNum =  Number(parameters.errorCorrectionLevel) || 1;
          if (errorCorrectLevelNum < 0) ecl = 0;
          if (errorCorrectLevelNum > 3) ecl = 3;
          switch (errorCorrectLevelNum) {
            // internally using a different scale
            case 0: errorCorrectLevel = 'L'; break; // internally 1
            case 1: errorCorrectLevel = 'M'; break; // internally 0
            case 2: errorCorrectLevel = 'Q'; break; // internally 3
            case 3: errorCorrectLevel = 'H'; break; // internally 2
          }
        }

        const qr = qrcode(version, errorCorrectLevel);
        qr.addData(code);
        qr.make();
        return {
          rows: qr.getModuleCount(),
          cols: qr.getModuleCount(),
          data: qr.getData(),
          parameters: {
            errorCorrectionLevel: errorCorrectLevelNum,
            version,
          },
        };
      };
    ...
    const create_image_data = (function () {
      ...
      return function (width, height, getPixel) {
